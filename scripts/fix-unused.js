...
try {
  ...
  const eslintCommand = "npx eslint . --ext .js,.jsx,.ts,.tsx --rule 'no-unused-vars: error' --rule '@typescript-eslint/no-unused-vars: error' --format json";
  ...
  report += 'This report identifies all unused variables in the codebase that should be addressed.\n\n';
  ...
  unusedVarsResults.forEach(file => {
    ...
    const unusedMessages = file.messages.filter(msg => 
      msg.ruleId === 'no-unused-vars' || 
      msg.ruleId === '@typescript-eslint/no-unused-vars'
    );
    ...
    report += '- Consider using TypeScript\'s "noUnusedLocals" and "noUnusedParameters" compiler options\n';
    report += '- For props or function parameters, use destructuring to pick only what you need\n\n';
  });
  ...
  console.log(`Analysis complete. Found ${totalUnused} unused variables in ${unusedVarsResults.length} files.`);
