...
try {
  ...
  const checks = [
    ...
    {
      name: 'Heavy conditional rendering',
      regex: /{\s*\w+\s*\?\s*\(\s*<[^>]*>\s*.*\s*<\/[^>]*>\s*\)\s*:\s*\(\s*<[^>]*>/gs,
    },
  ];
  ...
  const componentsWithIssues = Object.entries(componentIssues).filter(
    ([_, issues]) => issues.length > 0
  );
  ...
  if (componentsWithIssues.length === 0) {
    ...
    componentsWithIssues.forEach(([filePath, issues]) => {
      ...
      issues.forEach(issue => {
        const exampleFormatted = issue.examples[0].replace(/\n/g, ' ').replace(/\|/g, '\\|');
  ...
  report +=
    '1. **Component Memoization**: Use `React.memo` for functional components that render often with the same props\n';
  ...
  console.log(
    `Analysis complete. Found ${totalIssues} potential performance issues in ${componentsWithIssues.length} components.`
  );
