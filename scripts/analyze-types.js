...
try {
  ...
  const grepCommand =
    "grep -r --include='*.ts' --include='*.tsx' -E '(: any|any\\[\\]|Array<any>|<any,|,any>|<any>)' --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=app-backup --exclude-dir=app-consolidated .";
  ...
  report +=
    'This report identifies all usages of `any` types in the codebase that should be replaced with proper types.\n\n';
  ...
  Object.entries(fileMatches).forEach(([file, matches]) => {
    ...
    report +=
      '- Use `unknown` instead of `any` when the type is truly dynamic, then use type guards\n\n';
  });
  ...
  console.log(
    `Analysis complete. Found ${lines.length} instances of 'any' types in ${Object.keys(fileMatches).length} files.`
  );
