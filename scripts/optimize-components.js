...
try {
  ...
  componentFiles.forEach(filePath => {
    ...
    const componentExportMatch = content.match(
    ...
    const calculations = content.match(
      /(?:const|let)\s+(\w+)\s*=\s*(?:.*\.map\(|.*\.filter\(|.*\.reduce\()/g
    );
    if (calculations) {
    ...
    const handlers = content.match(
      /(?:const|let)\s+(\w+)\s*=\s*(?:useCallback\()?\s*\(\s*(?:\w+(?:\s*,\s*\w+)*)?\s*\)\s*=>/g
    );
    if (handlers) {
      const nonCallbackHandlers = handlers.filter(h => !h.includes('useCallback'));
      componentOptimizations[relativeFilePath].useCallbackForHandlers = nonCallbackHandlers.map(
        handler => {
    ...
    if (effects) {
      componentOptimizations[relativeFilePath].fixEffectDependencies = effects
        .map((effect, index) => {
    ...
    if (functionsInsideComponent) {
      componentOptimizations[relativeFilePath].moveFunctionsOutside = functionsInsideComponent.map(
        func => {
  ...
  Object.entries(componentOptimizations).forEach(([filePath, optimizations]) => {
    ...
    const needsOptimization =
      optimizations.memoizeComponent ||
