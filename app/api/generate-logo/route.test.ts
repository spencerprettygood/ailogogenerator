...
class MockReadableStream {
  ...
  constructor(options?: {
    start?: (controller: { enqueue: (chunk: unknown) => void; close: () => void }) => void;
  }) {
    this.controller = {
...
describe('Logo Generation API Route', () => {
  ...
  describe('POST handler', () => {
    it('should stream logo generation results', async () => {
      // Setup cache miss
      vi.mocked(CacheManager.getInstance().getGenerationResult).mockReturnValue(
    ...
    it('should return cached results when available', async () => {
      ...
      vi.mocked(CacheManager.getInstance().getGenerationResult).mockReturnValue(
        Promise.resolve(mockCachedResult)
      );
