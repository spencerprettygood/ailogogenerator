      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <Card className="p-4">
          ...
          <p className="text-xs mt-2 text-muted-foreground">
            This will make two identical API calls. The first should be a cache miss, the second a cache hit.
          </p>
        </Card>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => fetchCacheStatus(activeTab === 'detailed')} disabled={isLoading} className="flex-1">
                Refresh Status
              </Button>
              <Button variant="destructive" onClick={clearCache} disabled={isLoading} className="flex-1">
                Clear All Caches
              </Button>
            </div>
            <div className="grid grid-cols-2 gap-2 mt-4">
              <Button size="sm" variant="secondary" onClick={() => clearSpecificCache('generation')} disabled={isLoading}>
                Clear Generation
              </Button>
              <Button size="sm" variant="secondary" onClick={() => clearSpecificCache('intermediate')} disabled={isLoading}>
                Clear Intermediate
              </Button>
              <Button size="sm" variant="secondary" onClick={() => clearSpecificCache('asset')} disabled={isLoading}>
                Clear Assets
              </Button>
              <Button size="sm" variant="secondary" onClick={() => clearSpecificCache('progress')} disabled={isLoading}>
                Clear Progress
              </Button>
            </div>
          </div>
        ...
        <Card className="p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border rounded p-3">
              ...
              <p className="text-sm">Time: <span className="font-bold">{apiResponse.firstCall.time}ms</span></p>
              <p className="text-sm">Cache: <span className={`font-bold ${apiResponse.firstCall.cacheStatus === 'hit' ? 'text-green-600' : 'text-amber-600'}`}>
                {apiResponse.firstCall.cacheStatus}
              </span></p>
            </div>
            <div className="border rounded p-3">
              <h3 className="font-medium">Second Call (Cache Hit)</h3>
              <p className="text-sm">Time: <span className="font-bold">{apiResponse.secondCall.time}ms</span></p>
              <p className="text-sm">Cache: <span className={`font-bold ${apiResponse.secondCall.cacheStatus === 'hit' ? 'text-green-600' : 'text-amber-600'}`}>
