  ...
  const testModelConnection = async () => {
    ...
    try {
      const response = await fetch('/api/generate-logo', {
        ...
        body: JSON.stringify({
          prompt:
            'This is a test prompt to verify the Claude API connection. Please respond with a simple confirmation.',
          options: {
            model: selectedModel,
            temperature: 0,
            maxTokens: 50,
  ...
  return (
    <div className="container py-8">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <p className="mb-4">
            This tool will test the connection to the Claude API and verify that your API key is
            working properly.
          </p>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <p className="mb-4">Test a specific Claude model to verify it's available and working.</p>
      ...
      {diagnosticResult && (
        <Card>
          <CardHeader>
            <CardTitle>{diagnosticResult.success ? '✅ Success' : '❌ Failed'}</CardTitle>
          </CardHeader>
          <CardContent>
