...
export class TimeEstimator {
  ...
  endStage(stageId: string): void {
    ...
    if (startTime) {
      ...
      const configuredDuration = configuredStage
        ? configuredStage.duration
        : DEFAULT_STAGE_DURATIONS[normalizedStageId] || 10000;
  ...
  estimate(currentStageId: string, stageProgressPercent: number = 0): TimeEstimation {
    ...
    const currentStageConfiguredDuration =
      currentStageIndex >= 0 ? this.getStageEstimate(normalizedStageId) / 1000 : 0; // Convert to seconds
    const currentStageCompletedDuration =
      (stageProgressPercent / 100) * currentStageConfiguredDuration;

    // Calculate overall completion
    const totalCompletedConfiguredDuration =
      durationUpToCurrentStage + currentStageCompletedDuration;
    const completionPercentage =
      totalConfiguredDuration > 0
        ? (totalCompletedConfiguredDuration / totalConfiguredDuration) * 100
        : 0;
    ...
    if (elapsedSeconds > 0 && completionPercentage > 0) {
      ...
      dynamicallyEstimatedTotalDuration =
        observedTotalDuration * historyWeight + totalConfiguredDuration * (1 - historyWeight);
    ...
    const stageRemainingTime =
      currentStageConfiguredDuration > 0
        ? currentStageConfiguredDuration * (1 - stageProgressPercent / 100) * 1000 - stageElapsedMs
        : 0;
