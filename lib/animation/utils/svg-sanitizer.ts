    ...
    try {
      ...
      allElements.forEach(el => {
        for (let i = el.attributes.length - 1; i >= 0; i--) {
          ...
          const isDisallowed = SECURITY_CONFIG.disallowedAttributes.some(
            badAttr =>
              attrName === badAttr || attrName.startsWith(badAttr) || attr.value.includes(badAttr)
          );
...
function ensureViewBox(svgContent: string): string {
  ...
  if (!/viewBox\s*=/.test(svgContent)) {
    ...
    if (widthMatch && heightMatch) {
      ...
      return svgContent.replace(/(<svg[^>]*)>/, `$1 viewBox="0 0 ${width} ${height}">`);
    }
  }
...
function ensureSvgId(svgContent: string): string {
  ...
  if (!/id\s*=/.test(svgContent)) {
    ...
    return svgContent.replace(/(<svg[^>]*)>/, `$1 id="${id}">`);
  }
