...
export class StreamProcessor {
    ...
    try {
      ...
      while (!streamEnded) {
        try {
          ...
          callbacks.onError(
            new Error(
              `Stream read error: ${readError instanceof Error ? readError.message : String(readError)}`
            )
          );
          break;
        }
  ...
  private processLine(line: string, callbacks: StreamingCallbacks): void {
    ...
    try {
      ...
      try {
        ...
        console.debug(
          'Single JSON parse failed, attempting to process concatenated objects:',
  ...
  private processJsonData(data: any, callbacks: StreamingCallbacks): void {
    ...
    // Handle SVG preview
    if (
      (data.type === 'svg_preview' || data.type === 'preview') &&
      (data.previewSvg || data.preview)
    ) {
      try {
