...
import { TestVariant, TestComponent, InteractionEvent, FeedbackData, TestSession } from '../types';
...
export function useAbTest({
  ...
  const recordFeedback = useCallback(
    (metric: string, value: number | string | boolean, source: string) => {
      if (!sessionId) return;
  ...
  const completeTest = useCallback(
    (metrics: Record<string, any>, success: boolean = true) => {
      if (!sessionId) return;
