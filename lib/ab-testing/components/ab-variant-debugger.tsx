...
export const AbVariantDebugger: React.FC<AbVariantDebuggerProps> = ({
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 z-50 w-64 bg-gray-800 text-white p-3 rounded-lg text-xs opacity-80 hover:opacity-100 transition-opacity ${className}`}
    >
      ...
      <div className="space-y-1 mb-2">
        <p className="truncate">
          <span className="font-semibold">Test:</span> {testId}
        </p>
        <p className="truncate">
          <span className="font-semibold">Component:</span> {componentId}
        </p>
        <p className="truncate">
          <span className="font-semibold">Session:</span> {sessionId || 'Not assigned'}
        </p>
      </div>
