...
export const FeedbackCollector: React.FC<FeedbackCollectorProps> = ({
  ...
  const handleSubmit = () => {
    ...
    onSubmitFeedback(TestMetric.USER_SATISFACTION, rating, FeedbackSource.EXPLICIT_RATING);

    // Submit detailed feedback if provided
    if (showDetailedFeedback) {
      Object.entries(aspectRatings).forEach(([aspect, value]) => {
        onSubmitFeedback(`quality_${aspect}`, value, FeedbackSource.EXPLICIT_RATING);
      });
    }

    // Submit text feedback if provided
    if (feedbackText.trim()) {
      onSubmitFeedback('text_feedback', feedbackText, FeedbackSource.EXPLICIT_RATING);
    }
  ...
  if (submitted) {
    return (
      <div className={`bg-gray-50 rounded-lg p-4 mb-4 ${className}`}>
        ...
        <p className="text-sm text-gray-600">Your input helps us improve our logo generation.</p>
      </div>
    );
  }
    ...
    <div className={`bg-gray-50 rounded-lg p-4 mb-4 ${className}`}>
      ...
      {showDetailedFeedback && (
        <div className="mb-4 space-y-3">
          <p className="text-sm font-medium text-gray-700">
            Please rate these aspects of your logo:
          </p>
      ...
      <button
        ...
        className={`px-4 py-2 rounded-md text-white text-sm font-medium
          ${rating === null ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
      >
        Submit Feedback
      </button>
    </div>
