...
export class InputSanitizer {
  ...
  static validateSVG(svgContent: string): SecurityValidationResult {
    ...
    if (security_checks.has_use_element) {
      errors.push("SVG contains external use elements which may reference external content (medium severity).");
    }
    ...
    if (security_checks.size_exceeds_limit) {
      errors.push(`SVG exceeds maximum allowed size of 15KB (current size: ${Math.round(svgContent.length / 1024)}KB) (low severity).`);
    }
    ...
    if (security_checks.has_scripts || 
      ...
      severity = 'high';
    } else if (security_checks.has_external_refs || 
  ...
  static cleanSVG(svgContent: string): string {
    ...
    cleaned = cleaned.replace(/href\s*=\s*["']\s*data:(?:text\/html|application\/javascript)[^"']*["']/gi, 'href="#"');
    ...
    cleaned = cleaned.replace(/<use[^>]*href\s*=\s*["'][^#][^"']*["'][^>]*>(?:[\s\S]*?)<\/use>/gi, '');
