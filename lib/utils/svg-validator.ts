...
export class SVGValidator {
  ...
  static validate(svgContent: string): SVGValidationResult {
    ...
    for (const element of this.config.disallowedElements) {
      const pattern = new RegExp(`<${element}\\b[^<]*(?:(?!<\\/${element}>)<[^<]*)*<\\/${element}>`, 'i');
    ...
    for (const attrPattern of this.config.disallowedAttributes) {
      const pattern = typeof attrPattern === 'string' 
        ? new RegExp(`\\s${attrPattern}\\s*=\\s*["\'][^"\']*["\']`, 'i')
        : new RegExp(`\\s${attrPattern.source}\\s*=\\s*["\'][^"\']*["\']`, 'i');
      
      if (pattern.test(svgContent)) {
        securityChecks.has_event_handlers = true;
        errors.push(`SVG contains disallowed attribute: ${typeof attrPattern === 'string' ? attrPattern : attrPattern.source}`);
      }
    }
  ...
  private static calculateOptimizationScore(svgContent: string): number {
    ...
    if (size > 10 * 1024) { // Over 10KB
      score -= 15;
    } else if (size > 5 * 1024) { // Over 5KB
      score -= 5;
    }
  ...
  static repair(svgContent: string): SVGRepairResult {
    ...
    for (const element of this.config.disallowedElements) {
      const pattern = new RegExp(`<${element}\\b[^<]*(?:(?!<\\/${element}>)<[^<]*)*<\\/${element}>`, 'gi');
    ...
    for (const attrPattern of this.config.disallowedAttributes) {
      ...
      if (pattern.test(repairedSvg)) {
        repairedSvg = repairedSvg.replace(pattern, '');
        modifications.push(`Removed dangerous attributes: ${typeof attrPattern === 'string' ? attrPattern : attrPattern.source}`);
      }
    }
    
    // Remove external URLs and disallowed protocols
    for (const protocol of this.config.disallowedProtocols) {
      const protocolPattern = new RegExp(`\\s(?:href|src|data)\\s*=\\s*["']\\s*${protocol}[^"']*["']`, 'gi');
    ...
    if (!repairedSvg.includes('viewBox') && (repairedSvg.includes('width') && repairedSvg.includes('height'))) {
   ...
   */
  static process(svgContent: string, options: {
    repair?: boolean;
    optimize?: boolean;
  } = {}): {
    original: string;
