...
describe('SVGValidator', () => {
  ...
  describe('validate method', () => {
    ...
    it('should detect script elements as security issues', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.issues.some(i => i.type === 'security' && i.severity === 'critical')).toBe(
        true
      );
    ...
    it('should detect oversized SVGs', () => {
      ...
      expect(
        result.issues.some(
          i => i.type === 'optimization' && i.message.includes('exceeds maximum file size')
        )
      ).toBe(true);
    ...
    it('should detect event handlers as security issues', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.issues.some(i => i.type === 'security' && i.message.includes('disallowed attribute'))
      ).toBe(true);
    ...
    it('should detect external references with disallowed protocols', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.issues.some(i => i.type === 'security' && i.message.includes('disallowed protocol'))
      ).toBe(true);
      expect(result.securityScore).toBeLessThan(70);
    });
  });
  ...
  describe('repair method', () => {
    it('should repair SVGs with script elements', () => {
      ...
      expect(
        validationResult.issues.filter(i => i.type === 'security' && i.severity === 'critical')
          .length
      ).toBe(0);
    });
    ...
    it('should remove event handlers', () => {
      ...
      expect(repairResult.issuesFixed.some(i => i.message.includes('disallowed attribute'))).toBe(
        true
      );
      ...
      const validationResult = SVGValidator.validate(repairResult.repaired);
      expect(
        validationResult.issues.filter(i => i.type === 'security' && i.message.includes('onclick'))
          .length
      ).toBe(0);
    });
    ...
    it('should fix external references with disallowed protocols', () => {
      ...
      expect(repairResult.issuesFixed.some(i => i.message.includes('disallowed protocol'))).toBe(
        true
      );
      ...
      expect(
        validationResult.issues.filter(
          i => i.type === 'security' && i.message.includes('javascript:')
        ).length
      ).toBe(0);
    });
