  ...
  try {
    ...
    if (autoEnhance && originalAssessment.overallScore < minQualityThreshold) {
      ...
      const customizedOptions: DesignEnhancementOptions = {
        ...
        applyGoldenRatio:
          options.applyGoldenRatio !== false && originalAssessment.composition.score < 75,
        enhanceHierarchy:
          options.enhanceHierarchy !== false && originalAssessment.visualHierarchy.score < 75,
        optimizePaths:
          options.optimizePaths !== false && originalAssessment.technicalQuality.score < 80,
      };
    ...
    return (
      assessmentReport || {
        ...
        colorHarmony: {
          ...
          recommendations:
            validationResult.designQualityScore?.designSuggestions.filter(
              s => s.toLowerCase().includes('color') || s.toLowerCase().includes('palette')
            ) || [],
        },
        composition: {
          ...
          recommendations:
            validationResult.designQualityScore?.designSuggestions.filter(
              s => s.toLowerCase().includes('composition') || s.toLowerCase().includes('layout')
            ) || [],
        },
        visualHierarchy: {
          ...
          recommendations:
            validationResult.designQualityScore?.designSuggestions.filter(
              s => s.toLowerCase().includes('hierarchy') || s.toLowerCase().includes('weight')
            ) || [],
        },
        accessibility: {
          score: Math.round(
          ...
          recommendations:
            validationResult.designQualityScore?.designSuggestions.filter(
              s => s.toLowerCase().includes('contrast') || s.toLowerCase().includes('legibility')
            ) || [],
        },
        technicalQuality: {
          ...
          recommendations:
            validationResult.designQualityScore?.designSuggestions.filter(
              s => s.toLowerCase().includes('technical') || s.toLowerCase().includes('optimize')
            ) || [],
        },
      }
    );
