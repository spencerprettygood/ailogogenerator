        ...
        create: vi.fn().mockImplementation(async ({ messages }) => {
          ...
          if (typeof userMessage === 'string') {
            if (userMessage.includes('ByteWave')) {
              ...
              responseContent = JSON.stringify({
                brand_name: "FlexFit",
                brand_description: "Fitness program offering efficient 30-minute HIIT workouts for busy professionals",
...
describe('RequirementsAgent Integration Tests', () => {
  ...
  it('should successfully process tech startup brief', async () => {
    ...
    expect(result.result?.designSpec.target_audience).toBe('IT professionals and CIOs at mid to large enterprises');
    expect(result.result?.designSpec.style_preferences).toBe('Clean, modern, professional, minimalist');
  });
  ...
  it('should handle image descriptions in the input', async () => {
    const input: RequirementsAgentInput = {
      id: 'test-3',
      brief: 'I\'m launching \'FlexFit\' - a fitness program for busy professionals who want efficient workouts.',
      imageDescriptions: [
