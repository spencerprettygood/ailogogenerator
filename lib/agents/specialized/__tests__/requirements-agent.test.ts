...
describe('RequirementsAgent', () => {
  ...
  it('should detect industry from brief content', async () => {
    ...
    expect(detectIndustryMethod("We're opening a new restaurant in downtown")).toBe(
      'Food & Beverage'
    );
    expect(detectIndustryMethod('Our medical clinic needs a new brand identity')).toBe(
      'Healthcare'
    );
