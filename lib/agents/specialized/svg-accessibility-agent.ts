...
import {
  SVGAccessibilityValidator,
  SVGAccessibilityScore,
} from '../../utils/svg-accessibility-validator';
...
export class SVGAccessibilityAgent extends BaseAgent {
  constructor(config?: Partial<AgentConfig>) {
    super('svg-accessibility', ['svg-validation'], {
  ...
  protected async generatePrompt(input: SVGValidationAgentInput): Promise<string> {
    ...
    if (accessibilityScore) {
      ...
      if (accessibilityScore.colorContrast < 70) {
        issues.push(
          '- Improve color contrast for better visibility (current score: ' +
      ...
      if (accessibilityScore.textAlternatives < 70) {
        issues.push(
          '- Add appropriate text alternatives (title, desc, aria-label) (current score: ' +
            accessibilityScore.textAlternatives +
            '/100)'
        );
        needsAIImprovement = true;
      }
      ...
      if (accessibilityScore.semanticStructure < 70) {
        issues.push(
          '- Improve semantic structure for screen readers (current score: ' +
            accessibilityScore.semanticStructure +
            '/100)'
        );
        needsAIImprovement = true;
      }
      ...
      if (accessibilityScore.scalability < 70) {
        issues.push(
          '- Enhance scalability for better rendering at small sizes (current score: ' +
            accessibilityScore.scalability +
            '/100)'
        );
        needsAIImprovement = true;
      }
      ...
      if (accessibilityScore.interactiveElements < 70) {
        issues.push(
          '- Make interactive elements more accessible (current score: ' +
            accessibilityScore.interactiveElements +
            '/100)'
        );
        needsAIImprovement = true;
      }
      ...
      if (
        accessibilityScore.accessibilitySuggestions &&
        accessibilityScore.accessibilitySuggestions.length > 0
      ) {
        issuesPrompt +=
    ...
    if (
      !needsAIImprovement ||
      (accessibilityScore && accessibilityScore.overallAccessibility >= 80)
    ) {
...
Return only the improved SVG code without any explanations.`;
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          result: {
            ...
            designFeedback: this.generateAccessibilityFeedback(
              accessibilityResult.accessibilityAssessment
            ),
      ...
      return {
        ...
        result: {
          ...
          designFeedback: this.generateAccessibilityFeedback(
            finalAssessment.accessibilityAssessment
          ),
        },
        tokensUsed: this.metrics.tokenUsage.total,
        processingTime: this.metrics.executionTime,
  ...
  private applyAutomatedAccessibilityImprovements(svg: string, brandName: string): string {
    ...
    if (!/<title[^>]*>/i.test(improvedSvg)) {
      improvedSvg = improvedSvg.replace(/<svg/i, `<svg>\n  <title>${brandName} Logo</title>`);
    }
    ...
    if (!/<svg[^>]*role\s*=/i.test(improvedSvg)) {
      improvedSvg = improvedSvg.replace(/<svg([^>]*?)>/i, '<svg$1 role="img">');
    }

    if (!/<svg[^>]*aria-label\s*=/i.test(improvedSvg)) {
      improvedSvg = improvedSvg.replace(/<svg([^>]*?)>/i, `<svg$1 aria-label="${brandName} Logo">`);
    }
    ...
    improvedSvg = improvedSvg.replace(/<a([^>]*?)>/gi, (match, attributes) => {
  ...
  private generateAccessibilityFeedback(assessment?: SVGAccessibilityScore): string {
    ...
    if (overallAccessibility >= 90) {
      ...
      feedback =
        'Moderate accessibility. This SVG meets basic accessibility requirements but could be improved.';
    } else {
      feedback =
        'Poor accessibility. This SVG needs significant improvements to meet accessibility standards.';
    }
