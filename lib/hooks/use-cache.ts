...
export function useCache<T = any>(options: CacheOptions<T> = {}) {
  ...
  const [state, setState] = useState<CacheState<T>>(() => {
    try {
      ...
      if (storage === 'local' || storage === 'session') {
        ...
        const now = Date.now();
        if (memoryCache.current.meta.expiresAt !== 0 && 
            now > memoryCache.current.meta.expiresAt) {
          // Value has expired
  ...
  const setValue = useCallback((newValue: T | ((prev: T | null) => T)) => {
    try {
      setState(prevState => {
        // Calculate the new value
        const resolvedValue = typeof newValue === 'function' 
          ? (newValue as Function)(prevState.value) 
          : newValue;
...
export function useMemoizedAsync<T extends (...args: any[]) => Promise<any>>(
  ...
  const invalidate = useCallback((...args: Parameters<T>) => {
    const cacheKey = getCacheKey(args);
