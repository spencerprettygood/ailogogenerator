...
const testCallbacks = {
  onProgress: (progress) => {
    console.log(`PROGRESS: Stage ${progress.currentStage} - ${progress.stageProgress}% (Overall: ${progress.overallProgress}%)`);
  },
...
async function testSample(sample, index) {
  ...
  try {
    ...
    console.log(`Processing success rate: ${successRate}% (${processedCount}/${expectedObjectCount})`);
