...
const cardVariants = cva(
  // Base card styles
  ['relative overflow-hidden', 'transition-all duration-standard', 'bg-card text-card-foreground'],
  {
    variants: {
      variant: {
        // Default with subtle shadow and border
        default: ['border border-border shadow-sm', 'rounded-lg'],
        ...
        gradient: ['border-none rounded-lg shadow-sm', 'bg-asymmetric-gradient-1'],

        // Elevated with stronger shadow
        elevated: ['border border-border rounded-lg', 'shadow-asymmetric-lg'],
      },
      padding: {
        none: '',
        sm: 'p-3',
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn('font-heading font-thin text-2xl tracking-wider leading-tight', className)}
      {...props}
    />
  )
);
