...
const STAGE_INFO = {
  A: {
    ...
    description:
      'Analyzing your brand description to extract key design requirements including colors, style preferences, industry context, and target audience.',
    icon: 'analysis',
  },
  B: {
    title: 'Moodboard Creation',
    description:
      'Creating multiple design concepts based on your requirements, exploring different visual approaches, color schemes, and style directions.',
    icon: 'design',
  },
  C: {
    title: 'Concept Selection',
    description:
      'Evaluating all design concepts against your requirements to select the most effective direction for your brand identity.',
    icon: 'selection',
  },
  D: {
    title: 'Logo Generation',
    description:
      'Creating your custom SVG logo with precision vector graphics, crafting every element with attention to detail.',
    icon: 'generation',
  },
  E: {
    title: 'Logo Validation',
    description:
      'Optimizing your logo for performance and versatility, ensuring clean vector paths and technical quality.',
    icon: 'validation',
  },
  F: {
    title: 'Variant Creation',
    description:
      'Creating alternative versions of your logo for different contexts - monochrome variants, favicon, and multiple size formats.',
    icon: 'variants',
  },
  G: {
    title: 'Guidelines',
    description:
      'Developing comprehensive brand guidelines that document proper logo usage, color codes, and implementation recommendations.',
    icon: 'guidelines',
  },
  H: {
    title: 'Asset Packaging',
    description:
      'Preparing all your assets for convenient download - organizing files, creating appropriate formats, and packaging everything.',
    icon: 'packaging',
  },
  I: {
    title: 'Animation',
    description:
      'Adding smooth, engaging animations to your logo for digital applications, websites, and presentations.',
    icon: 'animation',
  },
};
...
export function ProgressTimeline({
  ...
  const getStageTitle = (stage: Stage) => {
    ...
    return (
      STAGE_INFO[stageKey as keyof typeof STAGE_INFO]?.title ||
      stage.label ||
      stage.name ||
      `Stage ${stage.id}`
    );
            ...
            <div className="space-y-1">
              {sortedStages.map((stage, index) => {
                ...
                const normalizedStatus =
                  stage.status === 'in-progress' ? 'in_progress' : stage.status;
                ...
                return (
                  <div key={stage.id} className="relative">
                    ...
                    <div className={`relative ${isCurrentStage ? 'z-10' : 'z-0'}`}>
                      <motion.div
                        initial={false}
                        animate={{
                              ...
                              <div className="pl-8 pr-2 pb-1">
                                ...
                                {normalizedStatus === 'in_progress' && (
                                  <div className="mt-3 bg-blue-50 dark:bg-blue-900/10 p-2 rounded text-xs">
                                    <span className="font-medium">Currently processing:</span> This
                                    stage is {Math.round(stage.progress)}% complete
                                  </div>
                                )}

                                {normalizedStatus === 'completed' && (
                                  <div className="mt-3 bg-green-50 dark:bg-green-900/10 p-2 rounded text-xs">
                                    <span className="font-medium">Completed:</span> This stage has
