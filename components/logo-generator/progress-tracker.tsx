...
const ProgressTracker: React.FC<ProgressTrackerProps> = ({
  ...
  const currentStageIndex = Array.isArray(stages)
    ? stages.findIndex(
        stage => stage && typeof stage === 'object' && 'id' in stage && stage.id === currentStageId
      )
    : -1;

  // Get current stage safely
  const currentStage =
    currentStageIndex >= 0 && currentStageIndex < stages.length ? stages[currentStageIndex] : null; // currentStage can be null if currentStageId is not found
  ...
  return (
    <Card className="w-full sticky top-4 shadow-lg dark:shadow-blue-900/50">
      <CardHeader className="pb-4">
        ...
        {currentStage &&
          (currentStage.status === 'in-progress' || currentStage.status === 'in_progress') && (
      ...
      <CardContent>
        ...
        <ul className="space-y-1.5 max-h-[calc(100vh-20rem)] overflow-y-auto pr-1">
          {stages.map((stage, index) => (
            <StageItem
              key={
                stage && typeof stage === 'object' && 'id' in stage ? stage.id : `stage-${index}`
              }
              stage={stage}
              isCurrent={
                stage && typeof stage === 'object' && 'id' in stage && stage.id === currentStageId
              }
              isCompleted={
                stage &&
                typeof stage === 'object' &&
