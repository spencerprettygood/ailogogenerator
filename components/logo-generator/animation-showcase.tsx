...
export const AnimationShowcase: React.FC<AnimationShowcaseProps> = ({
  ...
  const simpleAnimations = animationTemplates.filter(
    t =>
      t.id === 'fade-in' ||
      t.id === 'zoom-in' ||
      t.id === 'draw' ||
  ...
  const advancedAnimations = animationTemplates.filter(
    t => t.id === 'morph' || t.id === 'pulse' || t.id === 'sequential'
  );

  const interactiveAnimations = animationTemplates.filter(
    t =>
  ...
  const createPreview = (template: (typeof animationTemplates)[0]) => {
    ...
    const animatedSvg = svgPreview.replace(
      '<svg',
      `<svg id="${previewId}" class="${animationClass}"`
    );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {interactiveAnimations.length > 0 ? (
              ...
              <div className="col-span-full text-center p-8 text-gray-500 dark:text-gray-400">
                Interactive animations coming soon! These will respond to user actions like hover
                and click.
              </div>
            )}
          </div>
